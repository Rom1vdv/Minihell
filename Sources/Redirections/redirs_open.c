...
/*   Updated: 2023/01/23 10:59:33 by yhuberla         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	open_file(t_redirs *redir, t_ms *ms, t_enum mode)
{
	if ((mode == R || mode == RR))
	{
		...
		if (mode == R)
			ms->pipeout[1] = open(redir->file, O_WRONLY | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
		else
			ms->pipeout[1] = open(redir->file, O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
		if (ms->pipeout[1] == -1)
			perror(redir->file);
	}
